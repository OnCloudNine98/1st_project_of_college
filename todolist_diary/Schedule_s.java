...
// 이벤트 
	public void actionPerformed(ActionEvent ae) {
		...
		try {
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/smokedata?serverTimezone=UTC", "root",
					"dbsckdrl12");
			...
			if (ae.getSource() == s_search) { // s_search 엔터 의 경우 (검색)
				...
				strSql = "SELECT * FROM schedule WHERE name LIKE '%" + search_name + "%';";
				...
				while (r.next()) {
					...
					vNames.add(r.getString("name"));
					s_vYear.add(r.getString("start_year"));
					s_vMon.add(r.getString("start_month"));
					s_vDate.add(r.getString("start_date"));
					e_vYear.add(r.getString("end_year"));
					e_vMon.add(r.getString("end_month"));
					e_vDate.add(r.getString("end_date"));

					java.util.Calendar startCal = new GregorianCalendar(r.getInt("start_year"), r.getInt("start_month"),
							r.getInt("start_date"));
					java.util.Calendar endCal = new GregorianCalendar(r.getInt("end_year"), r.getInt("end_month"),
							r.getInt("end_date"));
...
////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void valueChanged(ListSelectionEvent se) { // 리스트 클릭 이벤트
		...
		try {
			...
			ResultSet r = dbSt.executeQuery(strSql);
			while (r.next()) {
				sNo.setText(r.getString("no"));
				schedule_name.setText(r.getString("name"));
				schedule_location.setText(r.getString("location"));
				sYear.setSelectedItem(r.getString("start_year"));
				sMonth.setSelectedItem(r.getString("start_month"));
				sDate.setSelectedItem(r.getString("start_date"));
				eYear.setSelectedItem(r.getString("end_year"));
				eMonth.setSelectedItem(r.getString("end_month"));
				eDate.setSelectedItem(r.getString("end_date"));
				memo.setText(r.getString("content"));
			}
