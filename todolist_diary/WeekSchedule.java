	...
	void printWeek(int a, int b, int c) {
		...
		try {
			...
			for (int i=Integer.parseInt(tmp), j=i;i<=j+6;i++) {
				String stqSql = "SELECT * FROM schedule WHERE '" + tmp +"' >= concat(schedule_start_year, schedule_start_month, schedule_start_date) and '" + tmp + "' <= concat(schedule_end_year, schedule_end_month, schedule_end_date)";//전달받은 날짜를 일요일로 기준삼아 토요일까지의 일정을 조회			
				...
				while(result.next()) {
					...
					txt.add(result.getString("location"));
					txt.add(result.getString("start_year")+"-"+result.getString("start_month")+"-"+result.getString("start_date"));
					txt.add(result.getString("end_year")+"-"+result.getString("end_month")+"-"+result.getString("end_date"));//기간 컬럼이 분리되어 있기 때문에 문자열 하나로 합쳐 표기
					txt.add(result.getString("content"));
